 ...
 * Designed for frontline employees who receive a link via SMS or QR code.
 * No CCLayout — completely standalone.
 *
 * Features (RR-036 + RR-037):
 *   - Fetches from GET /api/rr/shoutouts/public/:id (no auth required)
 *   - Beautiful mobile-first design, works on any phone
 *   - Shows: hero recipient section, value chips, message, sender, reactions
 *   - Share button copies the URL
 *   - QR code display for sharing further
...
function ReactionsSection({ shoutoutId, initialReactions }: {
  shoutoutId: string;
  ...
  async function handleReaction(emoji: string) {
    ...
    } catch {/* silent failure — optimistic update stays */}
...
export function FrontlinePage() {
  ...
  useEffect(() => {
    if (!id) { setError('Invalid recognition link.'); setLoading(false); return; }

    fetch(`${API}/api/rr/shoutouts/public/${id}`)
      .then(async r => {
        if (!r.ok) {
  ...
  function handleShare() {
    if (navigator.share) {
      navigator.share({ title: `${data!.recipientName} got recognized! 🎉`, url: pageUrl });
    } else {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          marginBottom: 24, fontSize: 12, color: 'rgba(255,255,255,0.8)',
      ...
      <div style={{ maxWidth: 480, margin: '-32px auto 0', padding: '0 16px 32px', position: 'relative' }}>
          ...
          <div style={{ padding: '28px 24px 20px' }}>
            ...
            <div style={{
              ...
              <div style={{
                ...
                background: '#dbeafe',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: 14, fontWeight: 700, color: '#1e40af', flexShrink: 0,
              }}>
        ...
        <div style={{
          ...
          {/* QR code — generated externally, no library needed */}
          <div style={{ display: 'inline-block', padding: 8, background: '#fff', borderRadius: 8, border: '1px solid #e5e7eb', marginBottom: 12 }}>
            <img
